"use client";

import { Button } from "@/components/ui/button";
import { useState } from "react";
import CreateCollectionSidebar from "@/components/dashboard/collection/collection-create-sidebar";
import { useMount } from "@/hooks/mount-hook";

const CreateCollectionButton = () => {
  const [open, setOpen] = useState(false);

  const mount = useMount();
  if (!mount) return null;

  const handleOpenChange = (open: boolean) => setOpen(open);

  return (
    <div className="w-full rounded-md bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 hover:to-yellow-800 p-[1px]">
      <Button
        onClick={() => setOpen(true)}
        variant="outline"
        className="dark:text-white w-full dark:bg-neutral-950 bg-white"
      >
        <span className="bg-gradient-to-r from-red-500 to-orange-500 hover:to-orange-800 bg-clip-text text-transparent">
          Create Collection
        </span>
      </Button>
      <CreateCollectionSidebar open={open} onOpenChange={handleOpenChange} />
    </div>
  );
};

export default CreateCollectionButton;
